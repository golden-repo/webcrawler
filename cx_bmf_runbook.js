...
class CheapPortal_Crawl {
    ...
    */
    ...
    async tx_read_action_controls(taskinfo) {
        var flag = true;
        var controls = [];
        var element_loaded = true;
        ...
        let contentsElements = await this.page.$$(selector).catch((reason)=> this.log('Read content : ', reason)).catch(reason => log(`E123 => ${reason}`));

        this.output_parameters.controls = (contentsElements && Array.isArray(contentsElements)) ? contentsElements : [];

        return (contentsElements && contentsElements.length>0);
    }
    ...
    */
    ...
    async tx_set_value(taskinfo) {
        var flag = true;

        var controls = this.input_parameters.controls;
        var controls_size = (controls && Array.isArray(controls)) ? controls.length : 0;
        var index = this.input_parameters.index;

        this.context.setContextData('controls', controls);
        this.context.setContextData('index', index);
        this.context.setContextData('controls_size', controls_size);
        this.log(`Control Size : ${controls_size} - Index : ${index}`);

        this.output_parameters.controls_size = controls_size;

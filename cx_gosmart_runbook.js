...
class GoSmartPortal_Crawl {
    ...
    */
    async tx_call_login_api(taskinfo) {
        var flag = true;
        var url = this.input_parameters.url;
        var method = this.input_parameters.method;
        var payloaddata = this.input_parameters.data;
        ...
        if(method.toUpperCase() == 'POST' && payloaddata !== undefined) {
        ...
        this.log('api result : ', JSON.stringify(data));

        this.output_parameters.data = data;
        this.output_parameters.user = data.user;
        this.output_parameters.access_token = data.access_token.replace('Bearer', '').trim();
        this.output_parameters.token_type = data.token_type;
        this.output_parameters.expires_in = data.expires_in;
        this.context.setContextData('token', this.output_parameters.access_token);

        return flag;
    }

    async tx_get_hot_deal(taskinfo) {
        var flag = true;
        var url = this.input_parameters.url;
        var method = this.input_parameters.method;
        var payloaddata = this.input_parameters.data;
        var token = this.context.getContextData('token'); // this.input_parameters.token;
        var content_type = this.input_parameters.content_type;
        var data = null;
        ...
        this.output_parameters.deals = Object.values(data);

        return flag;
    }

    async tx_get_deal_stock_dates(taskinfo) {
        var flag = true;
        var url = this.input_parameters.url;
        var method = this.input_parameters.method;
        var deals = this.input_parameters.deals;
        var payloaddata = this.input_parameters.data;
        var token = this.context.getContextData('token');
        var content_type = this.input_parameters.content_type;
        var data = null;
        ...
        for (let index = 0; deals != null && index < deals.length; index++) {
            let deal = deals[index];
            
            url = this.input_parameters.url.replace('${origin_id}', deal.destination.origin_id);
            url = url.replace('${destination_id}', deal.destination.destination_id);
            data = await this.hlp_fetch(url, options);

            this.log('api result : ', JSON.stringify(data));
            deal.stock_dates = data;
        }
        this.output_parameters.deals = deals;

        return flag;
    }

    async tx_get_tickets(taskinfo) {
        var flag = true;
        var url = this.input_parameters.url;
        var method = this.input_parameters.method;
        var deals = this.input_parameters.deals;
        ...
        for (let index = 0; deals != null && index < deals.length; index++) {
            let deal = deals[index];
            let stock_dates = deal.stock_dates;
            tickets = [];
            
            for (let idx = 0; idx < stock_dates.length; idx++) {
                const stock_date = moment(stock_dates[idx], "DD-MM-YYYY").format('MM-DD-YYYY');
                let destination = deal.destination.destination;
                let origin = deal.destination.origin;

                url = this.input_parameters.url.replace('${dpt}', stock_date.replaceAll('-', "%2F"));
                url = url.replace('${origin_id}', encodeURI(deal.destination.origin_id));
                url = url.replace('${destination_id}', encodeURI(deal.destination.destination_id));
                url = url.replace('${destination}', encodeURI(JSON.stringify({'id': destination.id, 'code': destination.code, 'name': destination.cityName})));
                url = url.replace('${origin}', encodeURI(JSON.stringify({'id': origin.id, 'code': origin.code, 'name': origin.cityName})));

                data = await this.hlp_fetch(url, options);
    
                this.log('api result : ', JSON.stringify(data));
                for (let ndx = 0; ndx < data.data.data.length; ndx++) {

                    try
                    {
                        let ticketItem = data.data.data[index];

                        this.log(`Ticket -> ${JSON.stringify(ticketItem)}`);
                        
                        tickets.push({
                            'arrival_date': ticketItem['arrival_date'],
                            'arrival_time': ticketItem['arrival_time'],
                            'departure_time': ticketItem['departure_time'],
                            'flight_no': ticketItem['flight_no'],
                            'id': ticketItem['id'],
                            'sell_price': ticketItem['sell_price'],
                            'travel_date': ticketItem['travel_date'],
                            'airline_id': ticketItem['airline_id']
                        });
                    }
                    catch(ex) {
                        this.log(`Error -> ${ex}`);
...
module.exports = GoSmartPortal_Crawl;
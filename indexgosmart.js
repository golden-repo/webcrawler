...
winston.configure({
    defaultMeta: {service: 'indexgosmart-crawler'},
    format: combine(label({label: 'gosmartcrawler'}), timestamp(), myFormat),
    transports:[
       new winston.transports.File({filename: `gosmart_execution_log_${moment().format("D_M_YYYY")}.log`, })
    ]
});
...
function load_rb_config(tenantname='') {
    ...
    switch (tenantname) {
        case 'gosmartflight':
            rb_file_name = './gosmartflight_crawl_runbook.json';
            break;
        default:
            break;
    }
...
async function ProcessActivityV2(targetUri, runid=uuid5()) {
    try
    {
        ...
        log('API base url set');
        //if(browser!==null && page!==null) {
            log('Base URL -> ' + targetUri);

            config_data = load_rb_config('gosmartflight');
            ...
            contextObj.setContextData('config', (config_data && config_data.config) ? config_data.config : null);
...
//     }
    ...
    try
    {
        ...
        let crawlingUri = "https://goflysmartapi.azurewebsites.net/api";

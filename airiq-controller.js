...
function init_routes(router) {
    router.get('/airiq/available-sectors', getAvailableSectors);
}
...
async function getAvailableSectors(req, res, next) {
    ...
    try
    {
        ...
        let payload =  null;
        let extraData = [
            {id: 1, key: 'workflowName', value: 'airiq:available-sectors'}
        ];
        ...
        airiqCrawlCommonLib.process(crawlingUri, payload, extraData, runid).then(async (data)=> {

...
class GFS_Crawl {
    ...
    async tx_get_token(taskinfo) {
        var flag = true;
        var url = this.input_parameters.url;
        var method = this.input_parameters.method;
        var email = this.input_parameters.email;
        var password = this.input_parameters.password;
        ...
        responsePayload = await fetch(url, options)
        .then(async response => {
            let content_type = response.headers._headers['content-type'][0]
        ...
        this.output_parameters.token = responsePayload.access_token.replace('Bearer', '').trim();
    ...
    */
    async tx_call_api(taskinfo) {
        ...
        availableSectors = await fetch(url, options)
        .then(async response => {
            this.log("info", "Response received");
            var rsp = response;
        ...
        this.output_parameters.availableSectors = availableSectors;

        return flag;
    }

    tx_transform_sector_data(taskinfo) {
        var flag = true;
        var token = this.input_parameters.token;
        var sectors = this.input_parameters.availableSectors;
        ...
        if(sectors && sectors.length>0) {
            for (let index = 0; index < sectors.length; index++) {
                ...
                try
                {
                    ...
                    if(sector.destinations && sector.destinations.length>0) {
                        for (let idx = 0; idx < sector.destinations.length; idx++) {
                            const sectorDestination = sector[idx];
                            
                            var tkt = {};
                            tkt.id = sectorDestination.id;
                            tkt.sector = "";
                            tkt.source = "";
                            tkt.destination = "";
                            tkt.availableDates = [];
